...
%test_image_paths  = cell(num_test_per_cat, 1);

train_labels = cell(num_categories * num_train_per_cat, 1);
%test_labels  = cell(num_test_per_cat, 1);
...
%    test_image_paths{(i-1)*num_test_per_cat + j} = fullfile(data_path, 'test', categories{i}, images(j).name);
